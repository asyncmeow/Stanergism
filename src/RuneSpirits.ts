...
import { RuneBlessingKeys, runeBlessings } from "./RuneBlessings"
...
export const getRuneSpiritPower = (spirit: RuneSpiritKeys): number => {
  ...
  export const buySpiritLevels = (spirit: RuneSpiritKeys, budget: Decimal) => {
    if (!achievementManager.getBonus('spiritUnlock')) {
      return
    }
  ...
  const updateLevelsFromEXP = (spirit: RuneSpiritKeys) => {
    ...
    const levels = Math.floor(levelsPerOOM * Decimal.log10(runeSpirits[spirit].runeEXP.div(runeSpirits[spirit].costCoefficient).plus(1)))
  ...
  const getLevelEstimate = (spirit: RuneSpiritKeys, offerings: Decimal) => {
    ...
    return Math.floor(runeSpirits[spirit].levelsPerOOM * Decimal.log10(totalEXP.div(runeSpirits[spirit].costCoefficient).plus(1)))
  }
  ...
  export const updateRuneEffectHTML = (spirit: RuneSpiritKeys) => {
    if (G.currentTab === Tabs.Runes) {
      DOMCacheGetOrSet(`${spirit}RuneSpiritPower`).innerHTML = runeSpirits[spirit].effectsDescription(getRuneSpiritPower(spirit))
    }
  }
